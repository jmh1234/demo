...
echo "curl --location --request PUT -w %{http_code} 'http://127.0.0.1:9001/api/stacks/6?endpointId=1' --header 'Authorization: Bearer ${value}' --header 'Content-Type: application/json' --data '{ \"id\": 6, \"StackFileContent\": \"${result6}\",\"Env\": [],\"Prune\": false }'" | sh
if [ $? -eq 0 ]; then
